...
return (
  <div className="min-h-screen bg-gradient-to-br from-slate-400 to-slat5-600">
    
    {/* Hero Section with Image Background */}
    <div
      className="text-white py-20 bg-cover bg-center"
      style={{
        backgroundImage: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url(${mountain})`
      }}
    >
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center justify-center gap-3 sm:static
    fixed bottom-4 right-4 z-50
    sm:justify-center">
            {!authLoading && user ? (
              <>
                <span className="text-sm text-slate-100 truncate max-w-[200px] border-b-2">
                  {user.email}
                </span>
                <button
                  type="button"
                  onClick={() => void signOut()}
                  className="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-red-300/50 hover:bg-red-400 transition-colors"
                >
        ...
        <p className="text-xl text-slate-300 text-center max-w-2xl mx-auto mb-8">
          Discover stories and philosophy from writers around the world
        </p>
